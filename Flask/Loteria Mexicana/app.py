...
# Ruta para el tablero de 4x4:
@app.route('/loteria')
def loteria_default():
    return generate_board(4, 4)

# Ruta para un tablero de 4 columnas y (x) filas
@app.route('/<int:x>')
def loteria_x(x):
    return generate_board(x, 4)

# Ruta para un tablero de (x) filas y (y) columnas
@app.route('/<int:x>/<int:y>')
def loteria_xy(x, y):
    return generate_board(x, y)

def generate_board(rows, cols):
    cartas_seleccionadas = sample(cartas, rows * cols)
    return render_template('tablero.html', rows=rows, cols=cols, cartas=cartas_seleccionadas)
